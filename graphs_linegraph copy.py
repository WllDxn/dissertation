...
def get_data(fname):
    ...
    # print(df)
    df = df.melt(id_vars=['data_type', 'data_size', 'cols', 'rows', 'shuffle'], var_name='method', value_name='times')
...
def g(df, fname):
    ...
    for sgroup, sg in sizeGroups:
        ...
        for tgroup, bg in typeGroups:
                ...
                print(bg)
                sns.lineplot(data=bg, x='shuffle', y = 'times', hue='methodbase',  estimator='mean', errorbar='pi')
                ...
                ticks = [x for x in bg['shuffle'].unique() if (int(x)%2)==0]
                ax.set_xticks(ticks)
...
if __name__ == '__main__':
    ...
    data.append(get_data(('/home/will/dissertation/sort_times/n_0_timsort_workingfinal_8.json')))
    gdata = pd.concat(data)
