            ...
            for idx, base in enumerate(new_dict[main_key]):
                if (
                    ...
                    if "rows" in list(base.keys()):
                        if base["rows"] == self.max_list_length and base["cols"] == self.max_list_count:
                            for key in list(self.data[x]["times"].keys()):
                                new_dict[main_key][idx]["times"].setdefault(key, [])
                                new_dict[main_key][idx]["rows"] = self.max_list_length
                            ...
                            break
            if not found:
                self.data[x]["rows"] = self.max_list_length
