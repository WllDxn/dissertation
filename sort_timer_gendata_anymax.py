...
def gen_list(cols, max_value, type="random"):
    max_value = int(pow(2, max_value)) - 1
...
class Sorter:
    def __init__(self, config):
        ...
        self.datasizes = range(5, 64, 2)
        self.datatypes = ["Random"]
        self.begin_sorting()
    ...
    def set_data(self, data, list_length, data_size, data_type, time):
        ...
        for idx, i in enumerate(dict):
            if (
                ...
                and i["data_size"] == data_size
                and i["rows"] == self.max_list_count
                and i["cols"] == list_length
            ):
    ...
    def begin_sorting(self):
        ...
        items = list(
            itertools.product(
                ...
                self.datasizes,
                self.datatypes,
                list(range(self.max_list_count)),
            )
        )
        interval = time.time()
        for list_length, data_size, data_type, count in items:
            ...
            self.print_sortmethod_count(
                data_size, data_type, count, len(items), interval
            )
            ...
            if count + 1 == self.max_list_count:
                self.print_sortmethod_evaluation(
                    interval, data_type, data_size, list_length
                )
                interval = time.time()
            self.total_count += 1
        with open(self.outputpath, "r+") as f:
    ...
    def print_sortmethod_count(
        ...
        myp("\033[K")
        myp(
            "\033[1;31m\r%s %s%s   Current: %d/%d%s%d/%d\r\t\t\t"
            % (
                data_type,
                data_size,
                datatabs,
    ...
    def print_sortmethod_evaluation(self, tdelta, data_type, data_size, list_length):
        ...
        myp(
            "\033[1;32m\r\033[K%s%stime: %f s  data size: %d"
            % (name, tabs, (time.time() - tdelta), list_length)
        )
        myp("\n")
