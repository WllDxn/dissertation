...
```
├── GUI 
|   GUI sorting interface
|
├── dissertation
|   project dissertation writeup
|
├── graphs
|   graphs of algorithm sort times
|
├── minor_files
|   Various utilities
|
├── radixsort_versions
|   Implementation of radix sort versions
|
├── sort_times
|   Results from sort time testing - used to produce graphs
|
├── sort_times_old
|   Old sort times, not guaranteed to be correct file structure
|
├── README.md
|   
├── compile.py
|   Compiles pypy with named version of radixsort
|
├── graphs.py
|   generate graphs
|
├── graphs_merge.py
|   generate graphs from multiple files
|
├── graphs_switch.py
|   unused
|
├── insertion_graphs.py
|   Generate graphs from testing insertion threshold
|
├── radixsort_profiler.py
|   profiles radix sort line by line
|
├── run_tests.py
|   Runs tests from multiple versions of pypy
|
├── sort_timer_gendata.py
|   Timer file called by run_tests.py
|
├── sort_timer_gendata_pipe.py
|   Timer file called by GUI
|
└── switch_bases.py
    Generates multiple versions of radix sort with different bases
```
## Graphs example
