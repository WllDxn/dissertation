...
def gen_list(cols, data_size, type="Random", threshold=None, insert=False, basemax=False, nearlysorted_percentage=10):
    # print(f'basemax {basemax}')
...
class Sorter:
    ...
    def set_data(self, data, list_length, data_size, data_type, time, threshold=None, shuffle=None):
        for entry in data["radix_sort"]:
            if (
                ...
                and (shuffle is None or shuffle == entry["shuffle"])
            ):
        ...
        if shuffle is not None:
            new_entry["shuffle"] = shuffle
        new_entry["times"] = {self.method: [time]}
    ...
    def begin_sorting(self):
        ...
        def generate_items():       
            ...
            for list_length in list(range(700000, self.max_list_length[0]+1, 20000)) if self.insert else self.max_list_length:
                for data_size in list(range(1, self.basemax, 1)) if self.basemax else self.datasizes:
                    for data_type in self.datatypes:
                        ...
                        for threshold in thresholds:
                            shuffle = [10] if self.shuffle is None else range(0, self.shuffle + 1, self.shuffle // 50)
                            for shuffle_val in shuffle:
                                for count in range(self.max_list_count):
                                    yield list_length, data_size, data_type, threshold, shuffle_val, count
        ...
        # print('shuff', sum(1 for _ in generate_items()))
        interval = 0
        sortd=True
        ...
        for list_length, data_size, data_type, threshold, shuffle, count in items:
            ...
            curr_list = gen_list(list_length, data_size, data_type, threshold, self.insert, self.basemax, nearlysorted_percentage=shuffle)
            if threshold:
            ...
            # print(f'shuff {shuffle}')
            data = self.set_data(data, list_length, data_size, data_type, newtime, threshold, shuffle if self.shuffle else None)
            interval += newtime
            if sortcount + 1 == self.max_list_count:
    ...
    def print_sortmethod_evaluation(
        ...
        ll = list(range(700000, self.max_list_length[0]+1, 20000)) if self.insert else [self.max_list_length]
        myp(
            "eval,%s,%f,%d,%s,%s"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="Perform testing of Nearly sorted lists with a variable shuffle factor (give a percentage)"
    )
