...
\newline
RPython utilises Arithmetic Shifting {\color{red}reference}, this means when a shift is performed and the rightmost bit is no longer in the rightmost position, all digits in the range $[0 .. s]$ are overwritten with the value 0 and an equal quantity of digits in the leftmost positions are discarded.
\newline
\newline
In this project, left shifting is used to create integer masks used to manipulate the sign of integers (further explained in \ref{sssec:xor}). 
\newline
...
\newline
Like leftshift, rightshift utilises Arithmetic shifting, meaning the rightmost $s$ digits are discarded. Rightshift differs from leftshift however as the digits in the the leftmost positions that become empty are overwritten with a value equal to the most significant (leftmost) bit, which would typically represent the sign of the integer. This means that a rightshifted integer retains it's original sign.
\newline\newline
Rightshifting is used in this project to manipulate integers from the input list in order to move a specified digit into the rightmost position so that the value of this digit can be extracted using Bitwise And (see \ref{sssec:and}). The bases selected for this project are of the order $2^n$ where $n$ is some integer, meaning a single digit of the integer occupies $n$ bits and therefore; rightshifting $n*d$ positions places the digit $d$ from the right in the rightmost position, occupying the rightmost $n$ bits. 
\pagebreak
\paragraph{And}\
\label{sssec:and}
\newline
The and operator takes two integers and compares the bits at each digit, giving a 1 if both digits are equal to 1 or 0 if either or both are equal to $0$.
\newline
 This is utilised in this project to isolate a certain number of digits in binary of an integer, specifically $n$ digits in binary represent one digit of the integer in base $2^{n}$. This is done by rightshifting $n*d$ positions (as stated in \ref{sssec:rightshift}) and then by performing an And operation with $2^{n}\minus1$, which in binary is represented as a $1$ in all digits in the leftmost $n$ positions, and 0 in all other positions, which will retrieve the rightmost $n$ bits and 0 for all other bits.
\newline
...
\label{sssec:xor}
\newline
Exclusive OR compares the bits of two integers and returns a value of 1 for each digit where the two bits are different and 0 if they are the same. It is performed in pypy using $x$ $\hat{}$ $y$ which results in an Exlusive Or performed across all 64 bits of the two integers $x$ and $y$. 
\newline
The use case for this in this project occurs when inverting the sign bit to ensure negative and positive values are sorted correctly (see \ref{ssec:negativestwos} for details).This is done by identifying how many digits in binary are needed to express the largest absolute value of the input (including a possible sign bit) using the function int\textunderscore bytes shown in \ref{ssec:intbytes} given as the integer $d$. Leftshifting an intger of value $1$ $d$ times and then subtracting 1 gives a value equal to the maximum integer expressable in the same number of bits as the max input value given as integer $v$. In binary $v$ contains $1$ in all digits up to the most significant digit of the max input value excluding any sign bits and a 0 in all other digits.
\newline
This value $v$ is then inverted using bitwise Not (see \ref{ssssec:not}) so that it instead contains $0$ in the bits representing the max input. As explained in \ref{ssec:negativesImplementation}, in two's complement binary all digits to the left of the most significant are equal and represent the sign of an integer, therefore $v$ contains $1$ in all the sign bits of the largest max input. Performing the Bitwise Exclusive Or operation with any value in the list against the value $v$ inverts all values in these sign bits. When sorting the input list in any given base, the leftmost digit has at least a single bit representing the sign in the leftmost position and after performing this exclusive or that bit is now 1 for positives and 0 for negatives and as such they will be sorted into the correct relative order.
\newline
...
\subsubsection{Illustratred example of Bitwise operations}
To provide further clarity on the matter of bitwise operations examples detailing each bitwise operation can be found below:
