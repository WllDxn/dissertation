...
\graphicspath{ {./demographs/} {./insertion/} {./results} {./lengthgraphs} {./basegraphs}}
\begin{document}
{
...
\par
Testing involving the comparison of the performance of algorithms including and excluding order checking showed that MSD radix sorts do sometimes create sorted sublists but skipping these sublists rarely results in a lower runtime as the cost of checking every list is too great. Due to the highly sporatic nature of benefits being seen, this type of order checking was not included.
...
\par  The decision to use \textit{perf\textunderscore counter} was made based on a reccomendation that is shown when using the \textit{timeit} module on PyPy, warning against the use of the module and instead presenting \textit{perf\textunderscore counter} as an alternative.
\par
...
\par
\begin{description}
	\item[Nearly sorted] lists generate a sorted list and repeatedly swap items at adjascent indexes until the number of swaps equals 10\% of the list length
...
As an extension of the sort check made at the start of each call to radix sort, it is possible to repeatedly check sublists for being sorted or reverse sorted in MSD sort and then mark these sublists as sorted to reduce further iterations of integer sorting being applied. Doing reduces runtime by reducing the amount of integer sorting required but increases runtime due to the time required to perform the order checks, this is further explained in \ref{sssec:order}.
\par
The function used to identify whether a list is sorted is shown below, and operates by iterating over an input list to identify whether every item is larger than or equal to the item at the previous index and then returns the booleans identifying whether the list is sorted or reverse sorted. The function also returns early if it is determined the list is neither sorted or rever sorted without iterating over every list item. 
\begin{lstlisting}	
...
\end{lstlisting}
\par
To evaluate the effectiveness of order checking, two versions of PyPy were compiled, one with the above function being used on every sublist and one without.{\color{red}These versions of PyPy were then tested on lists of $1$,$000$,$000$ integers within the largest possible range in the data formats of random, few unique and nearly sorted. The results of the tests show that in almost all cases the order checking has a detrimental effect or an extremely small positive effect and therefore this function was not included in this project.}
	...
	\par
	The continous testing of $n$ involved generating lists with a value for $n$ in the range $[1..1$,$000$,$000]$ and with list values of up to the maxmimum value for integers to ensure as best as possible that $d$ was constant. To test $d$, lists were generated with values constrained to the range $[-2^r..2^r\minus1]$ where $r$ varies from $[1..63]$, thereby increasing the value of $d$ but keeping $n$ constant.
	\par
	The non-continous testing evaluated the runtime of the algorithms of lists with a length of $10$,$000$, $100$,$000$ and $1$,$000$,$000$ and containing items with values within the range $[-2^{r}$ $..$ $2^{r}\minus1]$ where $r$ is $16$, $20$, $32$ and $63$. These parameters were selected to provide tests that represent the smallest and largest extremes of the continous tests as well as some tests in the middle. 
	\par
	Testing had to be split into continous and non-continous as performing a continous evaluation of both $n$ and $d$ would require a test of all values of $d$ performed for all values of $n$ and the amount of tests necessary makes this unfeasible. Furthermore, the data created by this would span 3 dimensions and would be difficult to extract useful information from. The continuous tests are useful for identifying the differences between each numerical base used for the algorithms, but as they are primarily evaluating the effect of $n$ and $d$ on Radix sort, they are not suitable for making a comparison to Timsort, the sort method used in \textit{listsort.py}.
	\par
	As the non-continous testing evaluates a range of both $n$ and $d$, the non-continous testing is also performed on lists generated in different format to evaluate each algorithms peformance in the widest range of circumstances. The formats used are identified in \ref{sssec:methodology} (Random, Nearly sorted and Few Unique). This type of testing was used to make a final comparison to Timsort, which is optimized for efficiency on lists with sorted sub-sections and therefore highly performant on nearly sorted lists.
	\par
	\pagebreak
	...
	\par To prevent the focus of this analysis being unfairly weighted towards lists of lengths closer to $1$,$000$,$000$ where the difference between run times is larger and therefore more visible on graphs, where the results may be differenct to lower list length, and which may not represent the most common use case for this algorithm, these tests were also performed with lists of length $100$,$000$. 
	...
	\par
	Base $6$ is the slowest shown, performing similarly to 8 and 16 around 100,000 items but falls off as more items are added. This bases seem to be too small to be suitable for this type of radix sort and this result is compounded in the ommitted bases 4 and 2 which was the worse performer.
	\par
	...
	\begin{table}[h]
		...
		\makebox[\linewidth]{
			...
			\caption*{MSD Counting sort length test for 100,000 and 1,000,000 items \par base 2 ommitted}
	\end{table}
	\pagebreak
	\myparagraph{MSD Pigeonhole sort}
	This set of results deviates qignificantly from the rest of this set, it shows that the larger bases are unsuitable for this implementation, with base 16 being the worst. This is the only iteration of this test in which base 2 wasn't the worst performer.
	\par
	...
	The results shown on this graph are so similar that any difference between them was completely unreadable if any of the other bases were not ommitted, showing that any of the bases 8,10,12,16 are suitable. As is typical for this type of testing, the jump in runtime as $data$\textunderscore $size$ reaches values divisible by each base is easily identifiable, showing the effect of each additional digit.
	\begin{table}[h]
		\centering
		\makebox[\linewidth]{
	...
	\end{table}
	\myparagraph{Random}
	Random data also was used for the continious tests and as expected, the results show similar results to the previous continuous tests for comparisons between the bases of each algorithm. 
	\par
	When comparing the algorithms against one another and timsort, the random data shows that all implementations of counting sort perform faster than timsort, as well as at least one base of each iteration of pigeonhole sort. The difference between the comparative nature of timsort and non-comparative radix sort is highlighted by observing LSD\textunderscore C for list length 1,000,000 (shown in figure \ref{fig:lrg_Random_0} and below); Timsort completes sorting in roughly $0.17s$ regardless of integer limit whereas LSD\textunderscore C sees smaller runtimes for smaller integer limits, showing the benefits of the fewer digits requiring counting sort in contrast with the lack of any reduction in the number of comparisons timsort must make.
	\begin{table}[h]
		\centering
		\makebox[\linewidth]{
			\includegraphics[draft=false,height=6cm]{lrg_Random_1.png}
			}
			\label{fig:lrg_Random_1}
	\end{table}

	\myparagraph{Few Unique}
	Data with few unique values was included simply to simulate the sorting of discrete values, which may be a potential use of a sorting algorithm in the real world. It was not expected to be drastically different to random data, and {\color{red} figure 2 and table 6} show this to be largely correct with the exception of certain higher data limits in longer lists for MSD sorts, particular counting sort. This could be due to the large number of items with the same digts preventing insertion sort from triggering but this only causes at most a 10\% difference to random.
	...
	\par
	The graph shown in {\color{red}figure 4} has been pruned to omit all of LSD\textunderscore P as well as bases 2, 14 and 16 as the results for these algorithms are much slower than timsort and make the graph unreadable. The remaining data is representative of all tests of this data type, with timsort performing exceptionally well compared to radix sort and being faster in every situation except on lists of 10,000 items where certain bases of MSD\textunderscore P and LSD\textunderscore C are a fraction faster.  
	\par
	Whilst timsort benefits from a 94\% decrease in runtime when performed on nearly sorted lists compared to random, radix sort typically benefits much less from the switch, as is illustrated in {\color{red} figure 5}. MSD sorts benefit from nearly sorted lists due to their use of insertion sort which completes in $\mathcal{O}\textit{(n)}$ time on sorted lists, which it will be called on frequently in this situation. LSD sorts do not employ this method and as such don't see much benefit. The discrepancy between pigeonhole and counting sort may be due to optimizations in PyPy involving moving items from one list into another in the same order, and there is some evidence for this such as LSD\textunderscore P having larger speed gains at smaller integer limits{\color{blue}, though I do not feel like any useful information could be gained from proving this theory}.
	\subsubsection{Testing evaluation}
	Making an empirical choice for the best algorithm is not possible in this situation, the varibles identified during time complexity analysis, $n$ and $d$, are independant of one another meaning an exhaustive performance test of both is not possible. Additionally, the categorical testing of different data types proves that the nature of the input list also affects sort times, meaning only sorting radom data would not paint the full picture. 
	\par
	...
	{\color{red} show table}
	\par
	Combining this information with the suitable bases identified in the continuous testing, the bases that have been selected for use are listed below, 36 situations were examined overall.
	\begin{description}
		\item[MSD Pigeonhole - 6] Performed well in 34 situations, tied with base 4 but showed greater performance in the variable test of length, particularly at smaller list lengths. Admittedly performed poorly for small integer sizes on large list lengths in the category few unique, but this is anomalous as this base performed very well in all other situations.
		\item[MSD Counting - 10] Deemed acceptable in 35 situations, with the exception being Few unique, medium int limit, length 1,000,000. This was selected over the best performer, base 4, which showed confusingly good results in categorical tests despite the poor performance in the continuous tests, for which it was rejected.
		\item[LSD Pigeonhole - 12] Bases 10, 12 and 14 were acceptable in all situations, and peformed very similarly on continuous tests so this decision was made arbritrarily so that the selection is more in line with the other methods.
		\item[LSD Counting - 10] Bases 6, 4, 14, 12 and 10 were all acceptable in all situations, but of those base 10 was the best performer in the continous tests.
	\end{description}
	\par
	...
	% \begin{table}[]
	% 	\begin{center}		
	% 	\caption*{Insertion sort tests for msd radix counting sort \hspace{.5cm}\includegraphics[width=.1\linewidth]{insertionLegend.png}}
	% 	\makebox[\linewidth][]{
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_2.png}
	% 		\end{minipage}%
	% 		\hspace{0.5cm}
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_4.png}
	% 		\end{minipage}
	% 		}\\
	% 	\makebox[\linewidth][]{
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_6.png}
	% 		\end{minipage}%
	% 		\hspace{0.5cm}
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_8.png}
	% 		\end{minipage}
	% 		}\\
	% 	\makebox[\linewidth][]{
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_10.png}
	% 		\end{minipage}%
	% 		\hspace{0.5cm}
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_12.png}
	% 		\end{minipage}
	% 		}\\
	% 	\makebox[\linewidth][]{
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_14.png}
	% 		\end{minipage}%
	% 		\hspace{0.5cm}
	% 		\begin{minipage}[t]{.55\linewidth}
	% 			\includegraphics[width=\linewidth]{msd_c_16.png}
	% 		\end{minipage}
	% 		}
	% 	\end{center}
	% \end{table}
	...
	% 	\makebox[\linewidth][]{
	% 	\centering
	% 	\includegraphics[width=1.2\linewidth]{lrg_Random.png}
	% 	}
	% 	\makebox[\linewidth][]{
	% 	\centering
	% 	\includegraphics[width=1.2\linewidth]{lrg_Nearly_Sorted.png}
	% 	}
	% 	\makebox[\linewidth][]{
	% 	\centering
	% 	\includegraphics[width=1.2\linewidth]{lrg_Few_Unique.png}
	% 	}
	% 	\makebox[\linewidth][]{
	% 	\centering
	% 	\includegraphics[width=1.2\linewidth]{lrg_Sorted.png}
	% 	}
	% 	\makebox[\linewidth][]{
	% 	\centering
	% 	\includegraphics[width=1.2\linewidth]{lrg_Reverse_Sorted.png}
	% 	}
	% \end{table}
