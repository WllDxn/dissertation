...
\pagenumbering{gobble}
    ...
    \includegraphics[draft=true,width=2.92358in,height=0.6639in]{logo}
    \vspace{2cm}
    \begin{LARGE}
      \linebreak
      COMP 390
...
\\	\\
	...
	The approach used in this project was to identify the number of digits needed to express the largest absolute value of the input in the chosen radixs and then use a bitwise exclusive or (\ref{sssec:xor}) to flip all bits to the left of it. The effect of this is that when performing a radix sort across all 64 bits of the integer list is that the order is now corrected as negative values closer to 0 retain their higher position relative to more negative values but are now always sorted below positive values.
	\par
...
\begin{table}[h]
	\captionsetup{type=figure}
	\makebox[\linewidth]{\includegraphics[width=1.2\linewidth]{democut}}
...
\begin{table}[h]
	\centering
	\makebox[\linewidth]{
	\includegraphics[ draft=true, height=5.5cm]{lrg_Nearly_Sorted_sorted_1.png}
	\hspace{0.25cm}
	\includegraphics[ draft=true, height=5.5cm]{lrg_Random_sorted_0.png}
	}
...
\end{table}
	...
	\begin{table}[h]
		\centering
		\makebox[\linewidth]{
			\includegraphics[ draft=true,height=4.8cm]{lsd_p_base}
		}
	...
	\begin{table}[h]
		\centering
		\makebox[\linewidth]{
			\includegraphics[ draft=true,height=4.8cm]{msd_c_base}
		}
	...
	\begin{table}[h]
		\centering
		\makebox[\linewidth]{
			\includegraphics[ draft=true,height=4.8cm]{msd_p_base}
		}
	...
	\begin{table}[b]
		...
		\captionsetup{type=figure}
		\makebox[\linewidth]{
			\includegraphics[ draft=true,height=6cm]{lrg_Random_0.png}
			\hspace{0.5cm}
			\includegraphics[ draft=true,height=6cm]{lrg_Random_1.png}
			}
			\vspace{-0.5cm}
	...
	\begin{table}[h]
		...
		\makebox[\linewidth]{
			...
			\begin{minipage}[h]{.55\linewidth}
				\captionsetup{type=figure}
				\hspace{.05\linewidth}
				\includegraphics[ draft=true, width=.9\linewidth]{med_Few_Unique_1.png}
				\caption{categorical tests for few unique data}
			\end{minipage}%
			\hspace{0.25cm}
			\begin{minipage}[h]{.55\linewidth}
				\resizebox{\columnwidth}{!}{%
				\begin{tabular}{llS[table-format=1.1]S[table-format=4.1]S[table-format=5.1]S[table-format=4.1]S[table-format=1.1]}
						
					\toprule
					items & int size & \multicolumn{1}{c}{lsd c} & \multicolumn{1}{c}{lsd p} & \multicolumn{1}{c}{msd c} & \multicolumn{1}{c}{msd p} & \multicolumn{1}{c}{timsort}\\
						\midrule
						\multirow[h]{4}{*}{$10^4$} & large & -9.5\% & 0.3\% & 2.3\% & 3.3\% & 6.9\% \\
						& med & -7.2\% & 6.1\% & 3.0\% & 3.6\% & 5.3\% \\
						& small & -3.4\% & 7.6\% & 6.3\% & 1.8\% & 1.1\% \\
						& tiny & -1.8\% & -0.3\% & 0.8\% & 6.5\% & 1.8\% \\
						\cline{1-7}
						\multirow[h]{4}{*}{$10^5$} & large & 4.7\% & -23.1\% & 18.2\% & 7.1\% & 5.2\% \\
						& med & 7.2\% & -10.4\% & 10.1\% & 9.0\% & 5.1\% \\
						& small & 1.7\% & 2.1\% & 2.0\% & 16.4\% & 3.1\% \\
						& tiny & 0.9\% & -5.6\% & 11.8\% & 1.0\% & 4.8\% \\
						\cline{1-7}
						\multirow[h]{4}{*}{$10^6$} & large & 8.8\% & -1.0\% & -124.9\% & 10.4\% & 7.0\% \\
				...
				\caption{Comparison of few unique and random data, using average between equivalent bases}
				\label{table:rnd-fu-diff}
			\end{minipage}
		}
	\end{table}
  ...
  table-align-text-post=false, % push ‘%’ next to the number
		...
		\makebox[\linewidth]{
			\begin{minipage}[h]{.5\linewidth}
				\captionsetup{type=figure}
				\hspace{.05\linewidth}
				\includegraphics[draft=true, width=.9\linewidth]{lrg_Nearly_Sorted_nolp_0.png}
				\caption{Nearly sorted data test with LSD\textunderscore P, bases 2, 14, 16 pruned\label{fig:lrg_Nearly_Sorted_nolp_0}}
			...
			\begin{minipage}[h]{.55\linewidth}
				\resizebox{\columnwidth}{!}{%
					...
					\begin{tabular}{llS[table-format=2.1]S[table-format=3.1]S[table-format=3.1]S[table-format=2.1]S[table-format=3.1]}
						
						\toprule
						items & int size & {lsd c} & \multicolumn{1}{r}{lsd p} & \multicolumn{1}{r}{msd c} & \multicolumn{1}{r}{msd p} & \multicolumn{1}{r}{timsort}\\
						\midrule
						\multirow[h]{4}{*}{$10^4$} & large & -3.1\% & -8.4\% & -1.3\% & 32.3\% & 91.9\% \\
						& med & -3.6\% & -9.7\% & 17.5\% & 52.0\% & 92.0\% \\
						& small & -4.5\% & -5.1\% & 0.9\% & 43.5\% & 92.2\% \\
						& tiny & -1.8\% & -3.9\% & 15.1\% & 39.6\% & 92.0\% \\
					   \cline{1-7}
					   \multirow[h]{4}{*}{$10^5$} & large & -0.2\% & 6.7\% & 8.6\% & 27.5\% & 94.4\% \\
						& med & -1.6\% & 10.8\% & 19.6\% & 43.1\% & 94.3\% \\
						& small & -3.7\% & 20.0\% & 16.4\% & 39.0\% & 94.3\% \\
						& tiny & -7.2\% & 32.4\% & 14.0\% & 31.1\% & 95.0\% \\
					   \cline{1-7}
					   \multirow[h]{4}{*}{$10^6$}& large & 0.3\% & 6.1\% & 35.4\% & 54.7\% & 95.3\% \\
				...
				\caption{Comparison of Nearly sorted and random data, using average between equivalent bases. Positives indicate nearly sorted is faster \label{table:rnd-ns-diff}}
		...
		\caption{Nearly sorted data test showing list length 10,000\label{fig:sml_Nearly_Sorted_nolp_1}}
	...
	\myparagraph{Selecting numerical base}
  ...
  table-space-text-post=\%}
		...
		\begin{tabular}{cS[table-format=3.0]S[table-format=3.0]S[table-format=3.0]S[table-format=3.0]}
			\toprule
			Method & \multicolumn{1}{c}{Few Unique} & \multicolumn{1}{c}{Nearly Sorted} & \multicolumn{1}{c}{Random} & \multicolumn{1}{c}{Average} \\
			\midrule
			lsd c & -88\% & 124\% & -88\% & -17\% \\
			lsd p & -46\% & 906\% & -42\% & 272\% \\
			msd c & -78\% & 181\% & -82\% & 6\% \\
