...
\graphicspath{ {./demographs/} {./insertion/}}
\begin{document}
{
...
\par
\begin{description}
	\item[Nearly sorted] lists generate a sorted list then iterates over the items and  for each has a $10\%$ chance to swap it with an item in the next index.
	\item[Few unique] generates a random list with a length $1/10^{th}$ of the target length then randomly populates a list of the target length with the preliminary list.
	\item[Random] generates list of random integers of the target length
\end{description} 
