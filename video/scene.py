...
class Countsort(Scene):
    def construct(self):
        ...
        for it in range(2):
            ...
            # txt = ([dv for dv in digvals])
            balltext = [f"{dv}" for dv in digvals]
            
            balltext = VGroup(*[Text(f"{dv}", font_size=26).next_to(d).set_x(diggroup.get_x()).shift(RIGHT*(0.5*(1+it))) for d, dv in zip(diggroup, digvals)])
            # return
...
class Bits(Scene):
    def construct(self):
        inp = 385643561
        self.play(FadeIn(Text(str(inp),font_size=40).to_edge(UP),shift=DOWN))
        base = 6
        basebits = int(ceil(log(inp, int(pow(2,base)))))
        bina = [(inp>>i)&1 for i in range((basebits*base)-1,-1,-1)]
        binatx = VGroup(*[Text(str(x), font_size=27) for x in bina]).arrange_in_grid(rows=1,cols=len(bina))
        ...
        basetx = Text(f'{int(pow(2,base))-1}', font_size=27).next_to(underline, DOWN, buff=0.25).set_x(binatx[-1].get_x()).shift(RIGHT*0.5)
        self.play(FadeIn(basetx))
        self.wait(0.5)
        valstx = VGroup(*[Text(str(x), font_size=27) for x in vals]).arrange_in_grid(rows=1,cols=len(vals)).next_to(underline,DOWN,buff=0.25).set_opacity(0)
        valstxbinary = VGroup(*[Text(str(1), font_size=27).move_to(valstx[i]) for i in range(len(vals))])
        self.play(Transform(basetx, valstxbinary, replace_mobject_with_target_in_scene=True))
        self.wait(0.5)
        
        valstx = valstx.set_opacity(1)
        self.play(Transform(valstxbinary, valstx, replace_mobject_with_target_in_scene=True))
        ...
        for i in range(basebits):
            ...
            target = [Text(str(x),font_size=27).next_to(o,DOWN) for x, o in zip(targetvals,valstx)]
            self.play(AnimationGroup(*[Transform(c,t) for c,t in zip(curr,target)]))
            targetsum = Text(str(sum(targetvals)),font_size=27).move_to(target[-1])
...
class Negatives(Scene):
    ...
    def construct(self):
        ...
        for idx, input in enumerate(inputs):
            ...
            binatx[idx] = VGroup(*[Text(str(x), font_size=27) for x in bina[idx]]).arrange_in_grid(rows=1,cols=len(bina[idx]))
        fullbina = VGroup(*[Text(str(x), font_size=27,stroke_width=1) for y in bina for x in y]).arrange_in_grid(rows=len(bina),cols=len(bina[0]))
        inputtxt = VGroup(*[Text(str(x), font_size=27) for x in inputs]).arrange_in_grid(rows=len(inputs),cols=1,col_alignments='r').to_edge(LEFT).shift(LEFT*1.25)
        ...
        for i in range(1, basebits+2):
            ...
            if i==1+(basebits):
                ...
                for j in range(len(inputs)):
                    ...
                    tmp = Text(str(bina[j][0]),font_size=27).move_to(fullbina[(basebits+1)*j])
