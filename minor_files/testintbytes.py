...
def make_radixsort_class(
    ...
    class Radixsort(object):
        ...
        def sort(self):
            ...
            if min_bytes == int_bytes((-sys.maxsize) - 1, self.base):
                uint_63 = ~((1 << bitno - 1) - 1)
            else:
                uint_63 = ~((1 << bitno) - 1)
            if bitno % self.base == 0 and bitno != int_bytes((-sys.maxsize) - 1, 1):
                min_bytes += 1

            counts = [[0 for _ in range(self.radix)] for _ in range(min_bytes)]
            print(bitno % self.base == 0, min_bytes)
...
arr = [176, 26, -123123, 2321,4353450,-9897]
r(arr).sort()
print(arr)
# for i in arr:
#     print(int_bytes(i, 4))
#     print(bin(i))
#     print(int_bytes(i, 1)%4==0)
