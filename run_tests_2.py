...
def n(methodName, output, num=10):
    ...
    rg = "[/.+?\./]"
    for name in fames:
        if not('lsd_c_8' in name):continue
        ...
        for l in [5000000]:
            exc = f"{pypy_versions}/{packagename}_{methodName}/bin/pypy sort_timer_gendata.py -m {packagename} -o {output} -n {num} -l {str(l)} -et 'Sorted' 'Nearly Sorted' 'Few Unique' 'Reverse Sorted'"
            print("\033[1;35m")
            print(exc)
            print("")
